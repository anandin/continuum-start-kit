    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-warm-hero p-6">
      <Card className="shadow-warm max-w-md">
        <CardContent className="py-12 text-center">
          ...
          <p className="text-sm text-muted-foreground mt-2">This profile may have moved.</p>
          <Button className="mt-6" onClick={() => navigate("/")}>Back to Haven</Button>
        </CardContent>
      </Card>
    </div>
  );
      ...
      <main className="container mx-auto px-4 py-12 max-w-3xl">
        <div className="text-center mb-10 animate-fade-in">
          <Avatar className="h-24 w-24 mx-auto mb-4 ring-4 ring-card shadow-warm">
            {data.avatarUrl && <AvatarImage src={data.avatarUrl} alt={data.providerName} />}
            <AvatarFallback className="bg-primary/10 text-primary text-2xl">
              {(data.providerName || data.title || "C").charAt(0)}
            </AvatarFallback>
          </Avatar>
          <h2 className="text-3xl font-semibold text-foreground">{data.providerName || data.title}</h2>
          {data.providerTitle && <p className="text-muted-foreground mt-1">{data.providerTitle}</p>}
        </div>
        ...
        <Card className="shadow-warm mb-6 animate-fade-in">
          <CardContent className="py-6">
            ...
            <h3 className="text-xl font-medium text-foreground mb-3">{data.title}</h3>
            <p className="text-foreground/80 leading-relaxed">{data.methodology}</p>
              ...
              <div className="space-y-3">
                {data.stages.map((s, i) => (
                  <div key={i} className="flex gap-3">
                    <div className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-primary/10 text-primary text-xs font-semibold">{i + 1}</div>
                    <div>
        ...
        <div className="text-center pt-4 animate-fade-in">
          <Button size="lg" onClick={() => navigate("/auth")} data-testid="button-connect" className="shadow-warm">

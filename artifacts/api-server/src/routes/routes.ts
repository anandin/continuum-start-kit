...
export function registerRoutes(app: Express) {
  ...
  app.post("/api/provider-config", requireProvider, async (req, res) => {
    try {
      ...
      const existingConfig =
        await storage.getProviderConfigByProviderId(userId);
  ...
  app.post("/api/provider-agent-config", requireProvider, async (req, res) => {
    try {
      ...
      const existingConfig =
        await storage.getProviderAgentConfigByProviderId(userId);

      if (existingConfig) {
        const updated = await storage.updateProviderAgentConfig(
          existingConfig.id,
          data,
        );
  ...
  app.get("/api/engagements/:id", requireAuth, async (req, res) => {
    try {
      ...
      if (!m.ok)
        return res
          .status(m.error === "Forbidden" ? 403 : 404)
          .json({ error: m.error });
    ...
    try {
      ...
      if (!m.ok)
        return res
          .status(m.error === "Forbidden" ? 403 : 404)
          .json({ error: m.error });
      const sessions = await storage.getSessionsByEngagementId(
    ...
    async (req, res): Promise<void> => {
      try {
        ...
        if (msg.role !== "seeker") {
          res
            .status(403)
            .json({ error: "Only seeker messages can be redacted" });
          return;
        }
        if (!msg.sessionId) {
          res.status(400).json({ error: "Message has no session" });
          return;
        }
        ...
        if (seekerUserId !== req.user!.id) {
          res
            .status(403)
            .json({ error: "Only the seeker may redact their own messages" });
          return;
        }
        if (msg.redactedAt) {
          res.json({ ok: true, alreadyRedacted: true });
          return;
        }
        ...
        if (attachmentRows.length > 0) {
          await Promise.all(
            attachmentRows.map((a) =>
              objectStorageService
                .deleteObjectEntity(a.storageKey)
                .catch((err) => {
                  req.log.warn(
        ...
        req.log.info(
          {
            messageId,
            cascadedMemoryCount: cascadedIds.length,
            attachmentBlobsDeleted: attachmentRows.length,
          },
  ...
  app.post("/api/chat", requireAuth, async (req, res) => {
    try {
      ...
      if (!typedText && attachmentInput.length === 0) {
        return res
          .status(400)
          .json({ error: "message or attachments required" });
      }
      ...
      if (seekerUserId !== req.user!.id) {
        return res.status(403).json({
          error:
            "Only the seeker may send messages on this endpoint. Providers cannot author Twin turns.",
        });
      }
      ...
      for (const a of attachmentInput) {
        const allowed =
          a.kind === "image" ? ALLOWED_IMAGE_MIME : ALLOWED_AUDIO_MIME;
        if (!allowed.has(a.mime.toLowerCase())) {
          return res
            .status(415)
            .json({ error: `Unsupported ${a.kind} type: ${a.mime}` });
        }
      }
      ...
      for (const a of attachmentInput) {
        ...
        if (!grant) {
          ...
          return res
            .status(403)
            .json({ error: "Attachment grant invalid or already consumed" });
        }
      }
      ...
      for (const a of attachmentInput) {
        ...
        if (
          stat.contentType &&
          a.kind === "image" &&
          !ALLOWED_IMAGE_MIME.has(stat.contentType.toLowerCase())
        ) {
          return res.status(415).json({
            error: `Uploaded blob content-type ${stat.contentType} not allowed for image`,
          });
        }
        if (
          stat.contentType &&
          a.kind === "audio" &&
          !ALLOWED_AUDIO_MIME.has(stat.contentType.toLowerCase())
        ) {
          return res.status(415).json({
            error: `Uploaded blob content-type ${stat.contentType} not allowed for audio`,
          });
        }
        stats.set(a.objectPath, {
      ...
      if (hasAudio && !transcriptionConfigured()) {
        return res.status(503).json({
          error:
            "Voice memo transcription is currently unavailable. Please send your message as text.",
        });
      }
      ...
      for (let i = 0; i < attachmentInput.length; i++) {
        ...
        try {
          const { buffer, contentType } =
            await objectStorageService.readObjectEntityBuffer(a.objectPath);
          ...
          req.log.warn(
            { err, sessionId, objectPath: a.objectPath },
            "voice memo transcription failed",
          );
          return res.status(422).json({
            error:
              "We couldn't transcribe that voice memo. Please try again or type your message.",
          });
        }
        if (!text) {
          return res.status(422).json({
            error:
              "We couldn't make out any speech in that recording. Please try again.",
          });
        }
      ...
      const transcriptParts = transcripts
        .map((t) => t.text)
        .filter((t) => t.length > 0);
      const photoCount = attachmentInput.filter(
        (a) => a.kind === "image",
      ).length;
      ...
      const seekerMsg = await db
        .transaction(async (tx) => {
          ...
          if (attachmentInput.length > 0) {
            ...
            const items: InsertMessageAttachment[] = attachmentInput.map(
              (a, i) => ({
                ...
                mime: a.mime,
                sizeBytes:
                  stats.get(a.objectPath)?.sizeBytes ?? a.sizeBytes ?? null,
                durationS:
                  a.kind === "audio"
                    ? (transcripts.find((t) => t.idx === i)?.durationS ??
                      a.durationS ??
                      null)
                    : null,
                transcript:
      ...
      if (!seekerMsg) {
        req.log.warn(
          { sessionId },
          "attachment grant race lost — no state persisted",
        );
        return res.status(409).json({
          error: "Attachment grant was consumed concurrently; please retry",
        });
      }
  ...
  app.post("/api/attachments/upload-url", requireAuth, async (req, res) => {
    try {
      ...
      if (seekerUserId !== req.user!.id) {
        return res
          .status(403)
          .json({ error: "Only the seeker may upload attachments" });
      }

      const allowed =
        body.data.kind === "image" ? ALLOWED_IMAGE_MIME : ALLOWED_AUDIO_MIME;
      if (!allowed.has(body.data.mime.toLowerCase())) {
        return res.status(415).json({
          error: `Unsupported ${body.data.kind} type: ${body.data.mime}`,
        });
      }
  ...
  app.get("/api/attachments/:id/url", requireAuth, async (req, res) => {
    try {
      ...
      const url = await objectStorageService.getObjectEntityDownloadURL(
        att.storageKey,
        3600,
      );
  ...
  app.post(
    ...
    async (req, res) => {
      try {
        ...
        if (!text) {
          return res.status(422).json({
            error: "We couldn't understand that recording. Try again.",
          });
        }
  ...
  app.post("/api/voice/tts", voiceJson, requireAuth, async (req, res) => {
    try {
      if (!ttsConfigured()) {
        return res
          .status(503)
          .json({ error: "Voice playback is not configured" });
      }
      ...
      if (!isSeeker) {
        return res
          .status(403)
          .json({ error: "Only the seeker can play coach audio" });
      }
      ...
      if (msg.role === "seeker") {
        return res
          .status(400)
          .json({ error: "Only coach replies can be spoken" });
      }
      ...
      if (!result) {
        return res
          .status(502)
          .json({ error: "Couldn't generate audio right now." });
      }
  ...
  app.post("/api/sessions/:id/finish", requireAuth, async (req, res) => {
    try {
      ...
      const providerConfig = await storage.getProviderConfigByProviderId(
      ...
      const stagesList = (
        (providerConfig.stages as ProviderConfigStage[] | null) ?? []
      )
        .map((s) => s.name)
        .join(", ");
...
Only return JSON. No extra text.`;
        ...
        messages: [
          {
            role: "system",
            content:
              "You are an expert coaching session analyst. Always respond with valid JSON only.",
          },
      ...
      const trajectory = (
        summaryData.trajectory_status ||
        summaryData.trajectoryStatus ||
        ""
      ).toLowerCase();
    ...
    try {
      ...
      if (!auth.ok)
        return res
          .status(auth.error === "Forbidden" ? 403 : 404)
          .json({ error: auth.error });
      const summary = await storage.getSummaryBySessionId(
        req.params.id as string,
      );
...
If ambiguous, choose the earliest relevant stage.`;
      ...
      try {
          ...
          messages: [
            {
              role: "system",
              content:
                "You are a stage assignment expert. Always respond with valid JSON only.",
            },
            { role: "user", content: prompt },
          ],
          temperature: 0.3,
        ...
        if (guarded.templated) {
          return res.json({
            initial_stage: stages[0]?.name || "Initial",
            rationale: "Default assignment (safety gate)",
          });
        }
      ...
      await logSafetyEvent({
        ...
        outputSnippet: internalAuditSnippet(
          content,
          "onboarding_assign_output",
        ),
        templateUsed: null,
        agentVersionId: null,
      });
    ...
    try {
      ...
      if (!auth.ok)
        return res
          .status(auth.error === "Forbidden" ? 403 : 404)
          .json({ error: auth.error });
      const indicators = await storage.getProgressIndicatorsByEngagementId(
        req.params.id as string,
      );
  ...
  async function assertProviderOwnsEngagement(
    ...
    if (engagement.providerId !== req.user!.id)
  ...
  async function assertSessionMember(
    ...
    AuthOk<{ session: DbSession; engagement: Engagement }> | AuthFail
  > {
  ...
  app.get("/api/engagements/:id/notes", requireAuth, async (req, res) => {
    try {
      const check = await assertProviderOwnsEngagement(
        req,
        req.params.id as string,
      );
      if (!check.ok)
      ...
      const notes = await storage.getClientNotesByEngagementId(
        req.params.id as string,
      );
  ...
  app.put("/api/notes/:id", requireAuth, async (req, res) => {
    try {
      ...
      if (note.providerId !== req.user!.id)
        return res.status(403).json({ error: "Forbidden" });
      const updated = await storage.updateClientNote(req.params.id as string, {
  ...
  app.get("/api/engagements/:id/goals", requireAuth, async (req, res) => {
    try {
      const engagement = await storage.getEngagementById(
        req.params.id as string,
      );
      if (!engagement)
      ...
      const goals = await storage.getGoalsByEngagementId(
        req.params.id as string,
      );
  ...
  app.put("/api/goals/:id", requireAuth, async (req, res) => {
    try {
      ...
      if (goal.providerId !== req.user!.id)
        return res.status(403).json({ error: "Forbidden" });
      ...
      if (status === "completed" && goal.status !== "completed") {
        await storage.resolvePendingProgressForCompletedGoal(
          req.params.id as string,
          req.user!.id,
        );
      }
  ...
  async function assertSeekerOwnsGoal(
    ...
    const seeker = await storage.getSeekerById(engagement.seekerId);
    if (!seeker || seeker.ownerId !== req.user!.id)
      return { ok: false, error: "Forbidden" };
  ...
  app.post("/api/goals/:id/seeker-progress", requireAuth, async (req, res) => {
    try {
      ...
      const existing = await storage.getPendingGoalProgress(
        goal.id,
        req.user!.id,
      );
      if (existing) {
  ...
  app.get(
    "/api/engagements/:id/goal-progress",
    requireAuth,
    async (req, res) => {
      try {
        const m = await assertEngagementMember(req, req.params.id as string);
        if (!m.ok)
          return res
            .status(m.error === "Forbidden" ? 403 : 404)
            .json({ error: m.error });
        const rows = await storage.getGoalProgressByEngagementId(
          req.params.id as string,
        );
  ...
  app.post("/api/goal-progress/:id/confirm", requireAuth, async (req, res) => {
    try {
      const progress = await storage.getGoalProgressById(
        req.params.id as string,
      );
      if (!progress)
      ...
      const result = await storage.confirmGoalProgress(
        progress.id,
        req.user!.id,
      );
      if ("error" in result) {
  ...
  app.get(
    "/api/intake-forms/by-provider/:providerId",
    requireAuth,
    async (req, res) => {
      try {
        const forms = await storage.getIntakeFormsByProviderId(
  ...
  app.put("/api/intake-forms/:id", requireAuth, async (req, res) => {
    try {
      const form = await storage.getIntakeFormById(req.params.id as string);
      if (!form) return res.status(404).json({ error: "Form not found" });
      if (form.providerId !== req.user!.id)
        return res.status(403).json({ error: "Forbidden" });
      const updated = await storage.updateIntakeForm(
        req.params.id as string,
        req.body,
      );
      return res.json(updated);
  ...
  app.post("/api/intake-responses", requireAuth, async (req, res) => {
    try {
      ...
      if (!intakeFormId || !engagementId || !answers)
      ...
      if (!form) return res.status(404).json({ error: "Form not found" });
      if (form.providerId !== engagement.providerId)
        return res
          .status(403)
          .json({ error: "Form does not belong to this provider" });
  ...
  app.get("/api/engagements/:id/intake", requireAuth, async (req, res) => {
    try {
      ...
      if (!allowed) return res.status(403).json({ error: "Forbidden" });
      const response = await storage.getIntakeResponseByEngagementId(
        req.params.id as string,
      );
  ...
  app.post("/api/resources", requireProvider, async (req, res) => {
    try {
      ...
      if (!["link", "document", "exercise"].includes(type))
  ...
  app.put("/api/resources/:id", requireProvider, async (req, res) => {
    try {
      ...
      if (resource.providerId !== req.user!.id)
        return res.status(403).json({ error: "Forbidden" });
  ...
  app.get("/api/engagements/:id/resources", requireAuth, async (req, res) => {
    try {
      ...
      if (!allowed) return res.status(403).json({ error: "Forbidden" });
      const assignments = await storage.getResourceAssignmentsByEngagementId(
        req.params.id as string,
      );
  ...
  app.post(
    "/api/resource-assignments/:id/view",
    requireAuth,
    async (req, res) => {
      try {
        const assignment = await storage.getResourceAssignmentById(
          req.params.id as string,
        );
        if (!assignment)
  ...
  app.post(
    "/api/coach/inbox/:engagementId/handle",
    requireProvider,
    async (req, res) => {
      try {
        ...
        await storage.dismissCoachInboxRow(
          req.user!.id,
          req.params.engagementId as string,
        );
        return res.json({ success: true });
        ...
        req.log.error(
          { err: error, engagementId: req.params.engagementId as string },
          "coach inbox dismiss failed",
        );
        return res.status(500).json({ error: error.message });
      }
    },
  ...
  async function deliverCoachMessage(
    ...
    if (seekerUserId) {
      ...
      try {
        ...
        req.log.warn(
          { err, engagementId },
          "push: coach message notify failed",
        );
      }
    }
  ...
  app.post(
    "/api/engagements/:id/coach-message",
    requireProvider,
    async (req, res) => {
      try {
        ...
        if (typeof message !== "string") {
          return res
            .status(400)
            .json({ error: "message (string) is required" });
        }
        const r = await deliverCoachMessage(
          req,
          req.params.id as string,
          message,
          {
  ...
  app.get(
    ...
    async (req, res) => {
      try {
        const chat = await storage.getActiveOnboardingChatByProviderId(
          req.user!.id,
        );
  ...
  app.post(
    ...
    async (req, res) => {
      try {
        ...
        let onboardingChat =
          await storage.getActiveOnboardingChatByProviderId(userId);
        if (reset || !onboardingChat) {
          if (onboardingChat) {
            await storage.updateProviderOnboardingChat(onboardingChat.id, {
              status: "completed",
            });
          }
        ...
        const history =
          (onboardingChat.messages as Array<{
            role: string;
            content: string;
          }>) || [];
...
Until [READY_TO_GENERATE], just keep the conversation going naturally. Do NOT generate the config — that's a separate step.`;
        ...
        if (history.length === 0) {
          aiMessages.push({
            role: "user",
            content:
              "Hi! I'm a new coach setting up my practice. Let's get started.",
          });
        }
        ...
        const aiContent =
          guardedChat.content || "Could you tell me more about your practice?";
        await logSafetyEvent({
          sessionId: null,
          engagementId: null,
          userId,
          ...
          inputSnippet: internalAuditSnippet(
            message,
            "provider_onboarding_chat_input",
          ),
          outputSnippet: internalAuditSnippet(
            aiContent,
            "provider_onboarding_chat_output",
          ),
          templateUsed: null,
          agentVersionId: null,
        });
        ...
        const updated = await storage.updateProviderOnboardingChat(
          onboardingChat.id,
          {
            messages: history,
          },
  ...
  app.post(
    "/api/provider-onboarding/generate",
    requireProvider,
    async (req, res) => {
      try {
        const userId = req.user!.id;
        const onboardingChat =
          await storage.getActiveOnboardingChatByProviderId(userId);
        if (!onboardingChat)
          return res.status(404).json({ error: "No active onboarding chat" });
        ...
        if (history.length < 4)
          return res
            .status(400)
            .json({ error: "Need more conversation first" });
...
Aim for 4-6 stages that reflect their actual journey. Use the coach's own language wherever possible.`;
          ...
          messages: [
            {
              role: "system",
              content:
                "You synthesize practice configs from coaching conversations. Always return valid JSON only.",
            },
            { role: "user", content: prompt },
          ],
          temperature: 0.4,
        ...
        if (guardedGen.templated) {
          return res.status(400).json({
            error:
              "Safety gate blocked config synthesis. Please revise the conversation.",
          });
        }
        ...
        await logSafetyEvent({
          ...
          inputSnippet: internalAuditSnippet(
            prompt,
            "provider_onboarding_generate_prompt",
          ),
          outputSnippet: internalAuditSnippet(
            content,
            "provider_onboarding_generate_output",
          ),
          templateUsed: null,
          agentVersionId: null,
        });
        ...
        try {
          ...
          return res.status(500).json({
            error:
              "AI returned invalid format. Please continue the conversation.",
          });
        }
  ...
  app.post(
    "/api/provider-onboarding/apply",
    requireProvider,
    async (req, res) => {
      try {
        const userId = req.user!.id;
        const chat = await storage.getActiveOnboardingChatByProviderId(userId);
        const config = (req.body.config ?? chat?.generatedConfig) as Record<
          string,
          unknown
        > | null;
        if (!config)
          return res.status(400).json({ error: "No config to apply" });
        ...
        if (existingProviderConfig) {
          await storage.updateProviderConfig(
            existingProviderConfig.id,
            providerConfigData,
          );
        } else {
        ...
        const agentData = {
          ...
          selectedModel:
            existingAgent?.selectedModel || "google/gemini-2.5-flash",
        };
        if (existingAgent) {
        ...
        if (chat) {
          await storage.updateProviderOnboardingChat(chat.id, {
            status: "completed",
          });
        }
  ...
  // ============================================================
  app.get(
    "/api/analytics/practice-overview",
    requireProvider,
    async (req, res) => {
      try {
        const userId = req.user!.id;
        ...
        const activeClients = engagements.filter(
          (e) => e.status === "active",
        ).length;
        ...
        for (const e of engagements) {
          ...
          for (const s of sessions) {
            ...
            if (summary?.assignedStage) {
              stageDistribution[summary.assignedStage] =
                (stageDistribution[summary.assignedStage] || 0) + 1;
            }
            if (
              summary?.trajectoryStatus &&
              trajectoryCounts[summary.trajectoryStatus] !== undefined
            ) {
        ...
        const avgSessionsPerClient = totalClients
      ...
      for (const e of engagements) {
        ...
        for (const g of goals) {
          if (
            g.status === "active" &&
            g.dueDate &&
            new Date(g.dueDate).getTime() < now
          ) {
            overdueGoals.push({
              engagementId: e.id,
              goalId: g.id,
  ...
  app.get("/api/public/provider/:providerId", async (req, res) => {
    try {
      ...
      const agent =
        await storage.getProviderAgentConfigByProviderId(providerId);
      if (!config && !agent)
  ...
  app.get("/api/twin/persona-examples", requireProvider, async (req, res) => {
    try {
      ...
      const playbookId =
        typeof req.query.playbookId === "string" ? req.query.playbookId : null;
      if (playbookId) {
        const pb = await getPlaybookById(playbookId);
        if (!pb || pb.providerId !== req.user!.id) {
          return res.status(404).json({ error: "Playbook not found" });
        }
        const rows = await listPersonaExamplesForPlaybook(
          req.user!.id,
          playbookId,
        );
        return res.json(rows);
  ...
  app.post("/api/twin/persona-examples", requireProvider, async (req, res) => {
    try {
      ...
      if (!scenario || !approvedResponse) {
        return res
          .status(400)
          .json({ error: "scenario and approvedResponse required" });
      }
  ...
  app.get("/api/twin/playbooks", requireProvider, async (req, res) => {
    try {
      ...
      const rows = await listPlaybooksByProvider(req.user!.id, {
        includeArchived,
      });
      res.json(rows);
    } catch (e: any) {
  ...
  app.patch("/api/twin/playbooks/:id", requireProvider, async (req, res) => {
    try {
      ...
      if (description !== undefined)
        patch.description = description?.slice(0, 2000) ?? null;
  ...
  app.post(
    ...
    async (req, res) => {
      try {
        ...
        if (!pb || pb.providerId !== req.user!.id)
          return res.status(404).json({ error: "Not found" });
        const row = await setDefaultPlaybook(
          req.user!.id,
          req.params.id as string,
        );
        return res.json(row);
  ...
  app.post(
    "/api/twin/playbooks/:id/duplicate",
    requireProvider,
    async (req, res) => {
      try {
        const pb = await getPlaybookById(req.params.id as string);
        ...
        const titleOverride =
          typeof req.body?.title === "string" && req.body.title.trim()
            ...
            : `${pb.title} (copy)`;
        const dup = await duplicatePlaybook(
          req.user!.id,
          req.params.id as string,
          titleOverride,
        );
  ...
  app.patch(
    ...
    async (req, res) => {
      try {
        ...
        const updated = await storage.updateEngagement(
          req.params.id as string,
          { playbookId: resolved },
        );
        return res.json(updated);
  ...
  app.use(async (req, res, next) => {
    ...
    if (req.path === "/api/review-queue" && req.method === "GET") {
      ...
      const labelMatch = req.path.match(
        /^\/api\/review-queue\/([^/]+)\/label$/,
      );
  ...
  app.post("/api/twin/calibration", requireProvider, async (req, res) => {
    try {
      ...
      const profile: SyntheticClientProfile = (syntheticClientProfile as
        | SyntheticClientProfile
        | undefined) ?? {
  ...
  app.post(
    ...
    async (req, res) => {
      try {
        ...
        const therapistAuthored =
          typeof req.body?.clientMessage === "string"
        ...
        if (therapistAuthored.length > 0) {
          ...
          try {
              ...
              messages: [
                {
                  role: "system",
                  content:
                    "You are a research-grade client simulator. Be realistic, not theatrical.",
                },
            ...
            return res
              .status(503)
              .json({ error: "Safety audit unavailable; please retry" });
          }
        }
  ...
  app.post(
    "/api/twin/calibration/:id/approve",
    requireProvider,
    async (req, res) => {
      try {
        const calib = await getCalibrationSession(req.params.id as string);
          ...
          await createPersonaExample(
            {
              ...
              rejectedResponse: isPositive
                ...
                : turn.draft,
              notes: null,
              tags: userTags.includes(labelTag)
                ? userTags
                : [...userTags, labelTag],
  ...
  app.post(
    ...
    async (req, res) => {
      try {
        ...
        const turn = transcript[turnIndex];
        const finalText =
          typeof approvedEdit === "string" && approvedEdit.length > 0
            ? approvedEdit
            : turn.draft;
  ...
  app.post(
    "/api/twin/calibration/:id/complete",
    requireProvider,
    async (req, res) => {
      try {
        const calib = await getCalibrationSession(req.params.id as string);
      ...
      try {
        ...
        if (!check.ok)
          return res
            .status(check.error === "Forbidden" ? 403 : 404)
            .json({ error: check.error });
        const rows = await listClientMemoryByEngagement(
          req.params.engagementId as string,
        );
        return res.json(rows);
      } catch (e: any) {
  ...
  app.delete("/api/twin/memory/:id", requireProvider, async (req, res) => {
    try {
      ...
      const ownerCheck = await assertProviderOwnsEngagement(
        req,
        mem.engagementId,
      );
      if (!ownerCheck.ok)
      ...
      if (mem.redactedAt)
        return res.status(410).json({ error: "Memory entry already redacted" });
  ...
  app.get(
    ...
    async (req, res) => {
      try {
        ...
        const rows = await listSafetyEventsByEngagement(
          req.params.engagementId as string,
          200,
        );
        return res.json(rows);
  ...
  app.post(
    "/api/twin/review-queue/:messageId/label",
    requireProvider,
    async (req, res) => {
      try {
        ...
        if (approvedEdit !== undefined && typeof approvedEdit !== "string") {
          return res
            .status(400)
            .json({ error: "approvedEdit must be a string" });
        }
        if (
          tags !== undefined &&
        ...
        const isPositive =
          safeLabel === "this_is_me" || safeLabel === "needs_edit";
        ...
        const defaultPb = reviewedEng.playbookId
          ? null
          : await getDefaultPlaybookForProvider(req.user!.id);
        const reviewPlaybookId =
          reviewedEng.playbookId ?? defaultPb?.id ?? null;
        await createPersonaExample(
          {
            providerId: req.user!.id,
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        if (!seeker) {
          return res
            .status(403)
            .json({ error: "Only seekers can log mood entries" });
        }
  ...
  app.get("/api/mood/me", requireAuth, async (req: Request, res: Response) => {
    try {
      ...
      const entries = await storage.getMoodEntriesBySeekerId(
        seeker.id,
        sinceDay,
      );
  ...
  app.get(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        const snapshot = await storage.getSeekerProgressSnapshot(userId, {
          moodWindowDays,
        });
        ...
        req.log.error(
          { err: error },
          "Failed to load seeker progress snapshot",
        );
        return res.status(500).json({ error: error.message });
      }
    },
  ...
  app.get(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        const days = clampDays(req.query.days as string, 14, 90);
        const sinceDay = shiftYmd(days - 1);
        const entries = await storage.getMoodEntriesByEngagementId(
          engagementIdParam,
          sinceDay,
        );
  ...
  const STARTER_PROMPTS: Array<{ text: string; category: string }> = [
    {
      text: "What's one thing that surprised you this week?",
      category: "weekly",
    },
    {
      text: "Describe a moment today when you felt fully present.",
      category: "daily",
    },
    {
      text: "What is one small step you can take tomorrow toward something that matters to you?",
      category: "daily",
    },
    {
      text: "What feeling has been hardest to sit with this week, and what does it want you to know?",
      category: "weekly",
    },
    {
    ...
    {
      text: "What's one thing you'd like to bring to your next session?",
  ...
  app.get(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        const includeArchived = req.query.includeArchived === "true";
        const prompts = await storage.listJournalPromptsForCoach(
          req.user!.id,
          includeArchived,
        );
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        if (!parsed.success) {
          return res
            .status(400)
            .json({ error: "Invalid prompt", details: parsed.error.flatten() });
        }
  ...
  app.patch(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        const updated = await storage.updateJournalPrompt(
          promptId,
          parsed.data,
        );
        return res.json(updated);
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        if (!seeker) {
          ...
          const starters = await storage.listAvailableJournalPromptsForSeeker(
            null,
            null,
          );
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        if (!seeker) {
          return res
            .status(403)
            .json({ error: "Only seekers can write journal entries" });
        }
        const bodySchema = z.object({
        ...
        if (!parsed.success) {
          return res
            .status(400)
            .json({ error: "Invalid entry", details: parsed.error.flatten() });
        }
        ...
        if (engagementId) {
          ...
          const seekerEngagements = await storage.getEngagementsBySeekerId(
            seeker.id,
          );
          const active =
            seekerEngagements.find(
        ...
        if (promptId) {
          ...
          if (!available.some((p) => p.id === promptId)) {
            return res
              .status(403)
              .json({ error: "Prompt not available to you" });
          }
        }
      ...
      try {
        ...
        if (existing.sharedWithCoach) {
          if (typeof body === "string" && body !== existing.body) {
            return res
              .status(409)
              .json({ error: "Entry is already shared and cannot be edited" });
          }
  ...
  app.get(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        const entries =
          await storage.listSharedJournalEntriesByEngagementId(
            engagementIdParam,
          );
        return res.json(entries);
      } catch (error: any) {
  ...
  app.get(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        const { generateOrFetchTodaysNudge } =
          await import("../services/nudges");
  ...
  app.get(
    "/api/seeker/nudge-prefs",
    requireAuth,
    async (req: Request, res: Response) => {
      try {
        const seeker = await storage.getSeekerByOwnerId(req.user!.id);
        if (!seeker) return res.status(403).json({ error: "Seekers only" });
        const { getNudgePrefs, DEFAULT_NUDGE_PREFS } =
          await import("../services/nudgeStorage");
        const prefs = await getNudgePrefs(req.user!.id);
        return res.json(
          prefs ?? { userId: req.user!.id, ...DEFAULT_NUDGE_PREFS },
        );
      } catch (error: any) {
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        if (!parsed.success) {
          return res
            .status(400)
            .json({ error: "Invalid prefs", details: parsed.error.flatten() });
        }
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        if (!parsed.success) {
          return res
            .status(400)
            .json({ error: "Invalid action", details: parsed.error.flatten() });
        }

        const { getNudgeById, respondToNudge } =
          await import("../services/nudgeStorage");
  ...
  // ============================================================
  app.post(
    "/api/engagements/:id/briefs/generate",
    requireProvider,
    async (req, res) => {
      try {
        ...
        req.log.error(
          { err: error },
          "POST /api/engagements/:id/briefs/generate failed",
        );
        return res.status(500).json({ error: error.message });
      }
    },
  ...
  app.get(
    "/api/engagements/:id/briefs/latest",
    requireProvider,
    async (req, res) => {
      try {
        const engagementId = String(req.params.id as string);
        ...
        req.log.error(
          { err: error },
          "GET /api/engagements/:id/briefs/latest failed",
        );
        return res.status(500).json({ error: error.message });
      }
    },
  ...
  app.post("/api/briefs/:id/used", requireProvider, async (req, res) => {
    try {
      ...
      const auth = await assertProviderOwnsEngagement(
        req,
      ...
      if (parsed.data.sessionId) {
        ...
        if (!session || session.engagementId !== existing.engagementId) {
          return res
            .status(400)
            .json({ error: "Session does not belong to this engagement" });
        }
      }
  ...
  function parseSlots(
    ...
    if (!Array.isArray(input))
      return { ok: false, error: "proposedSlots must be an array" };
    ...
    for (const raw of input) {
      if (typeof raw !== "string")
        return { ok: false, error: "slots must be ISO strings" };
  ...
  app.post(
    "/api/engagements/:id/scheduled-sessions",
    requireAuth,
    async (req, res) => {
      try {
        const check = await assertProviderOwnsEngagement(
        ...
        if (!seeker?.ownerId) {
          return res
            .status(400)
            .json({ error: "Engagement seeker has no owner" });
        }
        ...
        req.log.warn(
          { err: error },
          "POST /api/engagements/:id/scheduled-sessions failed",
        );
        return res.status(500).json({ error: error.message });
      }
    },
  ...
  app.get(
    ...
    async (req, res) => {
      try {
        const check = await assertEngagementMember(
          req,
          String(req.params.id as string),
        );
        if (!check.ok) {
        ...
        const { scheduledSessionStorage } =
          await import("../services/scheduledSessionStorage");
        const rows = await scheduledSessionStorage.listForEngagement(
          check.engagement.id,
        );
  ...
  app.get("/api/me/scheduled-sessions", requireAuth, async (req, res) => {
    try {
      ...
      const { scheduledSessionStorage } =
        await import("../services/scheduledSessionStorage");
      const { maybeFireReminderForSeeker } =
  ...
  app.post(
    ...
    async (req, res) => {
      try {
        ...
        const row = await scheduledSessionStorage.getById(
          String(req.params.id as string),
        );
        ...
        if (row.seekerUserId !== req.user!.id) {
          return res
            .status(403)
            .json({ error: "Only the seeker can confirm a slot" });
        }
        ...
        if (typeof body.slot !== "string") {
          return res
            .status(400)
            .json({ error: "slot (ISO string) is required" });
        }
        ...
        const proposedIso = ((row.proposedSlots ?? []) as unknown[]).map(
          (s) => {
            try {
        ...
        if (!proposedIso.includes(slotIso)) {
          return res
            .status(400)
            .json({ error: "slot is not one of the proposed times" });
        }
        ...
        // as 500 rather than silently allowing an unbilled booking.
        {
          ...
          const eb = await billingStorage.getEngagementBilling(
            row.engagementId,
          );
        ...
        try {
          ...
          if (!out.ok) {
            ...
            if (k !== "not_configured") {
              ...
              req.log.warn(
                { kind: k, id: row.id },
                "billing: per-session charge unsuccessful",
              );
            }
          }
        } catch (chargeErr: any) {
          ...
          billingWarning =
            "Payment couldn't be processed — please check your Payment tab.";
        }
        return res.json({ scheduledSession: updated, billingWarning });
      } catch (error: any) {
        req.log.warn(
          { err: error },
          "POST /api/scheduled-sessions/:id/confirm failed",
        );
        return res.status(500).json({ error: error.message });
      }
    },
  ...
  app.post(
    "/api/scheduled-sessions/:id/reschedule",
    requireAuth,
    async (req, res) => {
      try {
        const { scheduledSessionStorage } =
        ...
        req.log.warn(
          { err: error },
          "POST /api/scheduled-sessions/:id/reschedule failed",
        );
        return res.status(500).json({ error: error.message });
      }
    },
  ...
  app.post(
    ...
    async (req, res) => {
      try {
        ...
        req.log.warn(
          { err: error },
          "POST /api/scheduled-sessions/:id/cancel failed",
        );
        return res.status(500).json({ error: error.message });
      }
    },

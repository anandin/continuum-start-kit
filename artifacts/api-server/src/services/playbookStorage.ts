...
export async function listPlaybooksByProvider(
  ...
  return db
    ...
    .from(playbooks)
    .where(where)
    .orderBy(desc(playbooks.isDefault), desc(playbooks.createdAt));
}
...
export async function getDefaultPlaybookForProvider(
  providerId: string,
): Promise<Playbook | undefined> {
  const [row] = await db
    .select()
  ...
  return db.transaction(async (tx) => {
    if (data.isDefault) {
      await tx
        ...
        .where(
          and(
            eq(playbooks.providerId, data.providerId),
            eq(playbooks.isDefault, true),
          ),
        );
    }
...
export async function setDefaultPlaybook(
  providerId: string,
  playbookId: string,
): Promise<Playbook | undefined> {
  return db.transaction(async (tx) => {
    ...
    const [row] = await tx
      ...
      .where(
        and(eq(playbooks.id, playbookId), eq(playbooks.providerId, providerId)),
      )
      .returning();
    return row;
  });
...
export async function duplicatePlaybook(
  ...
  return db.transaction(async (tx) => {
    const [source] = await tx
      .select()
      .from(playbooks)
      .where(eq(playbooks.id, sourceId))
      .limit(1);
...
const STARTER_PLAYBOOKS: StarterPlaybook[] = [
  {
    ...
    description:
      "Warm onboarding for a brand-new client. Sets expectations, normalizes nervousness, invites them to share at their own pace.",
    examples: [
      {
        scenario:
          "I'm not really sure what to talk about. This is my first time.",
        approvedResponse:
          "That's a completely normal place to start. There's no right thing to bring in — sometimes it's easiest to just describe what your week has felt like, even in a few words. We can take it from there together.",
      ...
      },
      {
        ...
        approvedResponse:
          "It's fair to wonder. A lot of people show up unsure — and that uncertainty itself can be useful information. What would feel like a small sign, even a few weeks in, that this is worth your time?",
  ...
  },
  {
    ...
    description:
      "For ongoing clients dropping in between sessions. Keeps it grounded, returns to working themes, leaves room for them to lead.",
    examples: [
      {
        scenario: "Nothing major happened this week, just the usual stuff.",
        approvedResponse:
          "The 'usual stuff' is often where the most useful patterns live. What part of the week is sitting with you most right now — even if it feels small?",
        notes:
          "Don't accept 'nothing' — gently surface the texture of the week.",
        tags: ["check-in", "weekly"],
      },
      {
        scenario: "I've been doing okay, I think.",
        approvedResponse:
          "I hear the 'I think' in there. What would have to be true for you to feel sure?",
  ...
  },
  {
    ...
    description:
      "For clients sharing risky behaviors or moments of crisis. Stays calm, never lectures, prioritizes connection and safety planning over judgment.",
    examples: [
      {
        scenario: "I drank way more than I meant to last night.",
        approvedResponse:
          "Thank you for telling me. That takes some honesty. Before we get into what happened — how are you feeling right now, in your body and in your head?",
      ...
      },
      {
        ...
        approvedResponse:
          "I'm really glad you're telling me this. You're not in trouble for having that night — and you're not alone with it now. Are you safe at this moment? And if things tipped further, who is the first person you'd reach for?",
        notes:
          "Never moralize. Always check current safety + name a support contact.",
  ...
  },
  {
    ...
    description:
      "For clients reporting a win, a breakthrough, or a streak. Reflects it back specifically, lets them own the credit, anchors what made it work.",
    examples: [
      {
        scenario: "I actually went to the party and I didn't leave early!",
        approvedResponse:
          "That's huge — and it's not nothing. What was different this time about how you talked to yourself before going in?",
        notes:
          "Name the win specifically, then surface the mechanism so they can repeat it.",
        tags: ["celebration", "progress"],
      },
      {
        scenario: "I had my first full week without a panic attack in months.",
        approvedResponse:
          "A full week. I want to make sure that lands — months of carrying that, and this week was different. What do you think you did, even quietly, that helped?",
...
export async function seedStarterPlaybooksIfEmpty(
  providerId: string,
): Promise<Playbook[]> {
  const existing = await db
    .select({ id: playbooks.id })
    .from(playbooks)
    .where(eq(playbooks.providerId, providerId))
    .limit(1);
      ...
      examples: await Promise.all(
        sp.examples.map(async (ex) => ({
          ...
          embedding: await embed(
            ...
            logger.warn(
              { err, scenario: ex.scenario.slice(0, 60) },
              "starter example embed failed; will fall back to lexical retrieval",
            );
            return null;
          }),
        })),
  ...
  for (let i = 0; i < embeddedExamples.length; i++) {
    ...
    const playbook = await db.transaction(async (tx) => {
      ...
      for (const { ex, embedding } of examples) {
        ...
        await tx
          .insert(personaExamples)
          .values(embedding ? { ...base, embedding } : base);
      }
      return pb;
    });
  ...
  if (legacyUnscoped.length > 0 && created.length > 0) {
    ...
    logger.info(
      {
        providerId,
        defaultPlaybookId: defaultPb.id,
        migratedCount: legacyUnscoped.length,
      },

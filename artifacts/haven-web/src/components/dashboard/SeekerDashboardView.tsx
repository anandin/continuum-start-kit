...
export function SeekerDashboardView({ userId }: SeekerDashboardViewProps) {
  ...
  const handleProgressTileNav = useCallback(
    (kind: "sessions" | "goals" | "mood" | "streak") => {
      ...
      journeyCardRef.current?.scrollIntoView({
        behavior: "smooth",
        block: "start",
      });
    },
  ...
  const completedSessions = engagements.reduce(
    (acc, e) =>
    ...
    try {
      ...
      if (endedSessions.length > 0) {
        const res = await fetch(
          `/api/sessions/${endedSessions[0].id}/summary`,
          { credentials: "include" },
        );
        if (res.ok) {
  ...
  const handleStartChat = async () => {
    ...
    const activeSession = activeEngagements[0].sessions?.find(
      (s) => s.status === "active",
    );
    ...
    if (activeSession) {
      ...
      try {
        const latestSession = activeEngagements[0].sessions?.sort(
          (a, b) =>
            new Date(b.started_at).getTime() - new Date(a.started_at).getTime(),
        )[0];
        ...
        const res = await apiRequest("POST", "/api/sessions", {
          ...
          initialStage:
            latestSession?.initial_stage ||
            getLatestStage(activeEngagements[0]),
        });
  ...
  return (
    <div className="space-y-8 animate-fade-in">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground">
                  Pick your sliding-scale tier or view history
                </p>
              </div>
            </div>
          ...
          <CardContent className="py-4 flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground">
                  See and forget what your twin remembers about you
                </p>
              </div>
            </div>
            <ArrowRight className="h-4 w-4 text-muted-foreground" />
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-lg font-semibold text-foreground mt-1">
                  {activeEngagements[0]
                    ? getLastSessionDate(activeEngagements[0])
                    : "N/A"}
                </p>
              </div>
              <div className="h-11 w-11 rounded-xl bg-primary/10 flex items-center justify-center">
      ...
      <Card
        className="shadow-warm border-primary/10"
        data-testid="card-journal-cta"
      >
        <CardContent className="pt-6">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-muted-foreground">
                  Write between sessions. Share entries with your coach when
                  you're ready.
                </p>
              </div>
            </div>
      ...
      {hasEngagements && activeEngagements[0] && (
        <Card
          ref={journeyCardRef}
          className="shadow-warm-md border-primary/10 scroll-mt-24"
          data-testid="card-active-journey"
        >
          <CardHeader className="pb-4">
            <div className="flex items-center justify-between">
              ...
              <TrajectoryChip
                status={getTrajectoryStatus(activeEngagements[0])}
              />
            </div>
          </CardHeader>
          <CardContent className="space-y-5">
            ...
            {latestSummary && (
              <div className="space-y-4 pt-4 border-t border-border/60">
                {latestSummary.key_insights &&
                  latestSummary.key_insights.length > 0 && (
                    <div>
            ...
            {!loadingSummary && !latestSummary && (
              <div className="pt-4 border-t border-border/60">
                <p className="text-sm text-muted-foreground">
                  Complete your first session to unlock insights and progress
                  tracking.
                </p>
              </div>
            )}
      ...
      <Card
        ...
        data-testid="card-start-journey"
      >
        <CardContent className="pt-6">

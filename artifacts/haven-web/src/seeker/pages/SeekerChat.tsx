    ...
    async function init() {
      try {
        ...
        const active = Array.isArray(engagements)
          ? (engagements.find((e: any) => e.status === "active") ??
            engagements[0])
          : null;
        if (!active) throw new Error("No active engagement");

        // Try to find an existing active session, otherwise create one
  ...
  const sendMessage = useCallback(async () => {
    ...
    try {
      ...
      const botMsg: ChatMessage = {
        id: msg.id ?? `bot-${Date.now()}`,
        role: "twin",
        content: msg.content ?? msg.reply ?? "",

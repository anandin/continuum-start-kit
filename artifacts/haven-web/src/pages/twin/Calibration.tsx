...
const SCENARIOS = [
  {
    name: "Anxious about a job change",
    profile: {
      presenting: "considering leaving a stable job for a startup",
      tone: "anxious, second-guessing",
    },
  },
  {
    name: "Recurring relationship conflict",
    profile: {
      presenting: "same fight with partner keeps happening",
  ...
  },
  {
    ...
    profile: {
      presenting: "exhausted, overcommitted, can't say no",
      tone: "flat, depleted",
    },
  },
  {
    name: "Grief, six months in",
    profile: {
      presenting: "lost a parent six months ago",
      tone: "tender, sometimes numb",
    },
  },
];
...
export default function Calibration() {
  ...
  const sessionsQ = useQuery<CalibrationSession[]>({
    ...
    queryFn: async () =>
      (await apiRequest("GET", "/api/twin/calibration")).json(),
  });

  const activeQ = useQuery<CalibrationSession>({
    queryKey: ["/api/twin/calibration", activeId],
    queryFn: async () =>
      (await apiRequest("GET", `/api/twin/calibration/${activeId}`)).json(),
    enabled: !!activeId,
  });
  ...
  const turnMut = useMutation({
    mutationFn: async () =>
      (
        await apiRequest("POST", `/api/twin/calibration/${activeId}/turn`)
      ).json(),
    onSuccess: () =>
  ...
  const labelMut = useMutation({
    mutationFn: async (vars: {
      ...
      return (
        await apiRequest(
          "POST",
          `/api/twin/calibration/${activeId}/approve`,
          vars,
        )
      ).json();
    },
  ...
  const completeMut = useMutation({
    mutationFn: async () =>
      (
        await apiRequest("POST", `/api/twin/calibration/${activeId}/complete`)
      ).json(),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["/api/twin/calibration"] });
      qc.invalidateQueries({ queryKey: ["/api/twin/calibration", activeId] });
        ...
        <div className="flex justify-between items-center mb-6">
          <div>
            ...
            <p className="text-stone-600 text-sm">
              Practice the twin against synthetic clients. Your edits become the
              persona.
            </p>
          </div>
          <button
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="md:col-span-1">
            <div className="bg-white rounded-lg p-4 shadow-sm">
              ...
              <h3 className="font-medium text-stone-900 mt-6 mb-3">
                Past runs
              </h3>
                ...
                {sessionsQ.data?.map((s) => (
                  ...
                  >
                    ...
                    <div className="text-xs text-stone-500">
                      {(s.transcript || []).length} turns · {s.status}
                    </div>
                  </button>
                ))}
                {sessionsQ.data?.length === 0 && (
                  <p className="text-xs text-stone-500">
                    No runs yet — pick a scenario above.
                  </p>
                )}
              </div>
            </div>
            ...
            {activeId && activeQ.data && (
              <div className="bg-white rounded-lg p-4 shadow-sm">
                <div className="flex justify-between items-center mb-4">
                  <div>
                    <h2 className="font-medium text-stone-900">
                      {activeQ.data.scenarioName}
                    </h2>
                    <p className="text-xs text-stone-500">
                      Profile:{" "}
                      {JSON.stringify(activeQ.data.syntheticClientProfile)}
                    </p>
                  </div>
                ...
                <div className="space-y-4">
                  {(activeQ.data.transcript || []).map(
                    (t: CalibrationTurn, i: number) => (
                      <TurnCard
                        key={i}
                        turn={t}
                        index={i}
                        onLabel={(label, approvedEdit) =>
                          labelMut.mutate({ turnIndex: i, label, approvedEdit })
                        }
                  ...
                  {(activeQ.data.transcript || []).length === 0 && (
                    <p className="text-sm text-stone-500">
                      No turns yet. Click "Next turn" to begin the conversation.
                    </p>
                  )}
                </div>
              </div>
...
function TurnCard({
  ...
  index: number;
  onLabel: (label: string, approvedEdit?: string) => void;
  busy: boolean;
}) {
    ...
    <div className="border border-stone-200 rounded-lg p-3">
      ...
      <div className="mb-3">
        <div className="text-xs uppercase tracking-wide text-stone-500">
          Twin's draft{" "}
          {turn.templated && (
            <span className="ml-1 text-amber-700">(templated)</span>
          )}
        </div>
        {!editing ? (
      ...
      {!turn.label && (
        <div className="flex flex-wrap gap-2">
          <button
            disabled={busy}
            onClick={() => onLabel("this_is_me")}
            className="px-2 py-1 text-xs bg-emerald-100 hover:bg-emerald-200 text-emerald-900 rounded"
          >
            This is me
          </button>
          <button
          ...
          <button
            disabled={busy}
            onClick={() => onLabel("not_me")}
            className="px-2 py-1 text-xs bg-stone-200 hover:bg-stone-300 text-stone-800 rounded"
          >
            Not me
          </button>
          <button
            disabled={busy}
            onClick={() => onLabel("never_say_this")}
            className="px-2 py-1 text-xs bg-rose-100 hover:bg-rose-200 text-rose-900 rounded"
          >
            Never say this
          </button>
        </div>

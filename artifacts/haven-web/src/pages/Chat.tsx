  ...
  const loadSession = async () => {
    ...
    try {
      ...
      if (data.engagementId) {
        const engRes = await fetch(`/api/engagements/${data.engagementId}`, {
          credentials: "include",
        });
        if (engRes.ok) {
          ...
          if (engagement.providerId) {
            ...
            const configs = await fetch("/api/provider-configs", {
              credentials: "include",
            });
            if (configs.ok) {
              const allConfigs = await configs.json();
              const config = allConfigs.find(
                (c: any) => c.providerId === engagement.providerId,
              );
    ...
    >
                ...
                <div>
                  ...
                  {session?.initialStage && (
                    <p
                      ...
                      data-testid="text-session-stage"
                    >
                      {session.initialStage}
                    </p>
                  )}
          ...
          {messages.length === 0 ? (
            <div className="text-center py-20" data-testid="chat-empty-state">
              ...
              <p className="text-muted-foreground text-sm max-w-md mx-auto leading-relaxed">
                Take your time. Share whatever feels right. Your coach is here
                to listen and guide you on your journey.
              </p>
            </div>
          ) : (
            <>
                    ...
                    >
                      ...
                      >
                        {message.redactedAt ? (
                          <p
                            className="text-sm"
                            data-testid={`message-redacted-${message.id}`}
                          >
                            Message redacted
                            {redactedTime ? ` at ${redactedTime}` : ""}
                          </p>
                        ) : (
                          <>
                            {message.content ? (
                            ...
                            <p
                              className={`text-xs mt-2 ${message.role === "seeker" ? "opacity-70" : "text-muted-foreground"}`}
                            >
        ...
        <form onSubmit={handleSendMessage} className="max-w-3xl mx-auto">
          <div className="flex gap-2">
            ...
            <Button
              type="submit"
              disabled={
                sending || !inputMessage.trim() || isEnded || isProviderViewing
              }
          ...
          {isProviderViewing ? (
            <p className="text-xs text-muted-foreground text-center mt-2">
              You're viewing this conversation as the supervising therapist. The
              AI twin responds on your behalf — direct sends are disabled to
              preserve attribution. Use the Review Queue or Calibration to shape
      ...
      >
        <AlertDialogContent>
          ...
          <AlertDialogFooter>
            <AlertDialogCancel data-testid="button-cancel-redact">
              Keep it
            </AlertDialogCancel>
            <AlertDialogAction
              onClick={async () => {
                ...
                try {
                  await apiRequest(
                    "POST",
                    `/api/messages/${redactTarget.id}/redact`,
                  );
                  setMessages((prev) =>
                    prev.map((m) =>
      ...
      <AlertDialog open={showEndDialog} onOpenChange={setShowEndDialog}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Your conversation will be saved and a summary will be generated.
              You can always start a new session when you're ready.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>

...
import { Loader2, Plus, Trash2, ExternalLink, AlertTriangle, CheckCircle2 } from "lucide-react";
...
export default function ProviderBilling() {
  ...
  async function load() {
    ...
    try {
      const [s, t] = await Promise.all([
        fetch("/api/billing/connect/status", { credentials: "include" }).then((r) => r.json()),
        fetch("/api/billing/tiers", { credentials: "include" }).then((r) => r.json()),
      ]);
      setStatus(s);
      setTiers(t.tiers ?? []);
    } catch (e: any) {
  ...
  async function deleteTier(id: string) {
    if (!confirm("Archive this tier? Existing seekers on it keep their pricing.")) return;
    try {
          ...
          <>
              ...
              <CardContent className="space-y-4">
                {!status?.configured ? (
                  <div className="flex items-start gap-2 rounded-md bg-amber-50 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-900 p-3 text-sm">
                    ...
                    <div>Billing isn't yet enabled on this server. Once Stripe credentials are configured you'll be able to connect your Stripe account here.</div>
                  </div>
                ) : !status.stripeAccountId ? (
                  <>
                    <p className="text-sm text-muted-foreground">Connect a Stripe account to start receiving payments. We use Stripe Connect Express — Stripe handles tax forms, KYC, and payouts directly to your bank.</p>
                    <Button onClick={startOnboarding} disabled={onboarding} data-testid="button-stripe-onboard">
                      {onboarding ? <Loader2 className="h-4 w-4 mr-2 animate-spin" /> : <ExternalLink className="h-4 w-4 mr-2" />}
                  ...
                  <div className="space-y-3 text-sm">
                    <div className="flex items-center gap-2">
                      {status.chargesEnabled ? (
                        <Badge variant="default" className="gap-1"><CheckCircle2 className="h-3.5 w-3.5" /> Charges enabled</Badge>
                      ) : (
                        <Badge variant="secondary">Charges pending</Badge>
                      )}
                      {status.payoutsEnabled ? (
                        <Badge variant="default" className="gap-1"><CheckCircle2 className="h-3.5 w-3.5" /> Payouts enabled</Badge>
                      ) : (
                    ...
                    {!status.detailsSubmitted && (
                      <div className="flex items-start gap-2 rounded-md bg-amber-50 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-900 p-3">
                        <AlertTriangle className="h-4 w-4 text-amber-600 mt-0.5 shrink-0" />
                        <div>Stripe still needs more info before you can be paid. Continue onboarding below.</div>
                      </div>
                    )}
            ...
            <Card>
              <CardHeader>
                ...
                <p className="text-sm text-muted-foreground mt-1">Define the pricing options seekers can choose from. Per-session bills on each booked session; Monthly creates a Stripe subscription.</p>
              </CardHeader>
              <CardContent className="space-y-4">
                {tiers.length === 0 && (
                  <p className="text-sm text-muted-foreground italic">No tiers yet. Add your first below.</p>
                )}
                {tiers.map((t) => (
                  <div key={t.id} data-testid={`tier-row-${t.id}`} className={`flex items-center justify-between rounded-md border p-3 ${!t.isActive ? "opacity-50" : ""}`}>
                    <div>
                      <div className="font-medium text-foreground">{t.label} <span className="text-muted-foreground font-normal">— {fmtUsd(t.amountCents)} {t.billingCadence === "monthly" ? "/ month" : "/ session"}</span></div>
                ...
                <div className="rounded-md border bg-muted/30 p-4 space-y-3">
                  ...
                  <div className="grid sm:grid-cols-2 gap-3">
                    <div className="space-y-1.5">
                      ...
                      <Input id="tier-label" value={newLabel} onChange={(e) => setNewLabel(e.target.value)} placeholder="Standard" data-testid="input-tier-label" />
                    </div>
                    <div className="space-y-1.5">
                      <Label htmlFor="tier-amount">USD amount</Label>
                      <Input id="tier-amount" type="number" min="1" step="1" value={newAmount} onChange={(e) => setNewAmount(e.target.value)} data-testid="input-tier-amount" />
                    </div>

...
export default function Analytics() {
  ...
  const stageMax = data
    ? Math.max(1, ...Object.values(data.stageDistribution))
    : 1;
  const trajectoryTotal = data
    ? Object.values(data.trajectoryCounts).reduce((a, b) => a + b, 0)
    : 0;
  const sessionDates = data
    ? Object.entries(data.sessionsPerWeek)
  ...
  return (
    <AppLayout
      ...
      subtitle="A look at your work in numbers"
    >
      {isLoading ? (
        ...
        <div className="space-y-6">
          ...
          <Card className="shadow-warm animate-fade-in">
            <CardHeader>
              <CardTitle className="text-base">Trajectory mix</CardTitle>
            </CardHeader>
            <CardContent>
              {trajectoryTotal === 0 ? (
                ...
                <>
                  <div className="flex h-3 w-full overflow-hidden rounded-full bg-muted">
                    {Object.entries(data.trajectoryCounts).map(
                      ([k, v]) =>
                        v > 0 && (
                          <div
                            key={k}
                            className={trajectoryColor[k] || "bg-muted"}
                            style={{ width: `${(v / trajectoryTotal) * 100}%` }}
                            title={`${k}: ${v}`}
                          />
                        ),
                    )}
                  ...
                  <div className="mt-3 flex flex-wrap gap-3 text-xs">
                    {Object.entries(data.trajectoryCounts).map(([k, v]) => (
                      <div key={k} className="flex items-center gap-1.5">
                        <span
                          className={`inline-block h-2.5 w-2.5 rounded-full ${trajectoryColor[k] || "bg-muted"}`}
                        />
          ...
          <div className="grid gap-4 lg:grid-cols-2">
            <Card className="shadow-warm animate-fade-in">
              <CardHeader>
                <CardTitle className="text-base">Where clients are</CardTitle>
              </CardHeader>

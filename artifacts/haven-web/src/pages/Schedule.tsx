...
import { Loader2, AlertCircle, Clock, Target, CalendarDays } from "lucide-react";

interface ScheduleOverview {
  recentSessions: Array<{ engagementId: string; sessionId: string; startedAt: string; status: string }>;
  inactiveClients: Array<{ engagementId: string; lastActivity: string }>;
  overdueGoals: Array<{ engagementId: string; goalId: string; title: string; dueDate: string }>;
}
          ...
          <Card className="shadow-warm animate-fade-in">
            ...
            <CardContent className="space-y-2">
              {(data?.inactiveClients ?? []).length === 0 ? (
                <p className="text-sm text-muted-foreground">Everyone's engaged. Nice work.</p>
              ) : data!.inactiveClients.map(c => (
                <button
                  key={c.engagementId}
                  onClick={() => navigate(`/provider/engagement/${c.engagementId}`)}
                ...
                >
                  ...
                  <p className="text-xs text-muted-foreground">Last activity: {c.lastActivity ? new Date(c.lastActivity).toLocaleDateString() : "never"}</p>
                </button>
              ))}
            </CardContent>
          </Card>
          ...
          <Card className="shadow-warm animate-fade-in">
            ...
            <CardContent className="space-y-2">
              {(data?.overdueGoals ?? []).length === 0 ? (
                <p className="text-sm text-muted-foreground">No overdue goals.</p>
              ) : data!.overdueGoals.map(g => (
                <button
                  key={g.goalId}
                  onClick={() => navigate(`/provider/engagement/${g.engagementId}`)}
                  className="w-full text-left rounded-lg border border-border/60 p-3 hover:bg-muted/40 transition-colors"
          ...
          <Card className="shadow-warm animate-fade-in md:col-span-2 xl:col-span-1">
            ...
            <CardContent className="space-y-2">
              {(data?.recentSessions ?? []).length === 0 ? (
                ...
                <button
                  key={s.sessionId}
                  onClick={() => navigate(`/provider/engagement/${s.engagementId}`)}

...
export default function ProviderOnboarding() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await fetch("/api/provider-onboarding/status", {
          credentials: "include",
        });
        ...
        if (chat && Array.isArray(chat.messages) && chat.messages.length > 0) {
          ...
          const r = await apiRequest("POST", "/api/provider-onboarding/chat", {
            message: "",
          });
  ...
  const generate = async () => {
    ...
    try {
      ...
      toast({
        title: "Couldn't generate config",
        description: e.message,
        variant: "destructive",
      });
    } finally {
  ...
  const apply = async () => {
    ...
    setApplying(true);
    try {
      await apiRequest("POST", "/api/provider-onboarding/apply", {
        config: generated,
      });
      toast({
        title: "Practice set up!",
        description: "Your Haven space is ready.",
      });
          ...
          <Card className="shadow-warm flex flex-col h-[calc(100vh-12rem)] animate-fade-in">
            <CardHeader className="border-b border-border/60">
              ...
              <p className="text-sm text-muted-foreground">
                A short conversation. I'll learn how you work, then prepare your
                Haven space.
              </p>
            </CardHeader>
            ...
            <div className="border-t border-border/60 p-4 space-y-2">
              <div className="flex gap-2">
                ...
                <Button
                  onClick={send}
                  disabled={sending || !input.trim() || bootstrapping}
                ...
                <CardContent className="space-y-3 text-sm text-muted-foreground">
                  ...
                  <ul className="space-y-2">
                    <li className="flex items-start gap-2">
                      <Check className="h-4 w-4 text-primary mt-0.5" /> Your
                      program title and approach
                    </li>
                    <li className="flex items-start gap-2">
                      <Check className="h-4 w-4 text-primary mt-0.5" /> The
                      journey stages your clients move through
                    </li>
                    <li className="flex items-start gap-2">
                      <Check className="h-4 w-4 text-primary mt-0.5" /> Your
                      voice, tone, and boundaries
                    </li>
                    <li className="flex items-start gap-2">
                      <Check className="h-4 w-4 text-primary mt-0.5" /> An AI
                      assistant that sounds like you
                    </li>
                  </ul>
                </CardContent>
              </Card>
...
function ConfigPreview({
  ...
  return (
    <Card
      className="shadow-warm border-primary/30 animate-fade-in"
    ...
    >
      ...
      <CardContent className="space-y-4 text-sm max-h-[calc(100vh-20rem)] overflow-y-auto">
        ...
        <Field
          ...
          onChange={(v) => onChange({ ...config, methodology: v })}
          multiline
          testid="field-methodology"
        />
        <div>
